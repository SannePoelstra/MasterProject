...
#We want to create more stimuli, so instead of 6 foodgroups, I will have 12, so double that. They will still hold to the same outcome structure, so each experimental condition
#will have 4 instead of 2 foodgroups
...
Cues7 <- FillInCues("strawberries", "peanuts", "shrimp", Cue)
Cues8 <- FillInCues("bran", "cabbage", "yoghurt", Cue)
Cues9 <- FillInCues("chicken", "mustard", "bananas", Cue)
Cues10 <- FillInCues("walnuts", "peaches", "wheat", Cue)
Cues11 <- FillInCues("horseradish", "lobster", "corn", Cue)
Cues12 <- FillInCues("cheese", "pork", "blueberries", Cue)
